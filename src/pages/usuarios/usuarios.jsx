...
import {
  Container, TextField, Button, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Switch, Box,
  Dialog, DialogTitle, DialogContent, DialogActions, Typography, Snackbar, Alert, Checkbox, List, ListItem, ListItemText
} from '@mui/material';
import { Edit, Delete, Visibility, VisibilityOff, Info, Add } from '@mui/icons-material';
...
const Usuarios = () => {
  ...
  const [openAddCursosDialog, setOpenAddCursosDialog] = useState(false);
...
//'https://via.placeholder.com/50'
  ...
  const handleSaveCursos = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const errorData = await response.json();
        throw new Error(errorData.error || 'Error al asociar cursos al usuario');
      }
      await fetchUsuarios();
  ...
  return (
    <Container>
      ...
      <Dialog open={openInfoDialog} onClose={handleCloseInfoDialog}>
        ...
        <DialogContent>
          <Typography><strong>Nombre:</strong> {usuarioInfo?.usuario.nombre}</Typography>
          <Typography><strong>Email:</strong> {usuarioInfo?.usuario.email}</Typography>
          <Typography><strong>Estado:</strong> {usuarioInfo?.usuario.estado ? 'Activo' : 'Inactivo'}</Typography>
    ...
    <List>
      {cursos.map((curso) => (
        <ListItem key={curso._id} button onClick={() => handleCursosChange(curso._id)}>
          <Checkbox
            checked={selectedCursos.includes(curso._id)}
          />
